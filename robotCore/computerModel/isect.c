                 ...
                 int nrhs, const mxArray *prhs[])
{
    ...
    if (!(mxIsSingle(prhs[0])) || !(mxIsSingle(prhs[1]))) {
        mexErrMsgTxt("Input vertices must be of type single.");
    }
    ...
    /* Get the data */
    pr=(float *)mxGetPr(prhs[0]);
    pr2=(float *)mxGetPr(prhs[1]);
    result=tri_tri_intersect(&pr[0], &pr[3], &pr[6], &pr2[0], &pr2[3],&pr2[6]);   
    if(result)
    {
    }
    //TODO:  If there are two output arguments, return the intersection line if it exists.
    if(nlhs==1)
    {
        plhs[0]=mxCreateNumericMatrix(1,1,mxINT32_CLASS,mxREAL);
        res=(int *)mxGetPr(plhs[0]);
        *res=result;
    }
}
